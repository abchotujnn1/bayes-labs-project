...
import ast
import itertools
class Bio_Pathway_Dataset(Dataset):
    def __init__(self, path):
        # pdb.set_trace()
        with open(path) as file:
            self.data=pd.read_csv(file)
        self.data['num_atom'] = [Chem.MolFromSmiles(smile).GetNumAtoms() for smile in self.data['smile']]
        self.data = self.data[self.data['num_atom'] <= 40]
        unique_label= list(set(itertools.chain(*list(ast.literal_eval(i) for i in list(self.data['pathway'].values)))))
        self.data['y']=[[1 if l in ast.literal_eval(label) else 0 for l in unique_label] for label in list(self.data['pathway'].values)]
        self.data=self.data[['smile','y']]
